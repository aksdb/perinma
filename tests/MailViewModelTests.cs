...
{
    [Test]
    public async Task HydratedSelection_RemainsSelectedAfterBodyLoad()
    {
        ...
        var messageId = await storage.CreateOrUpdateMailMessageAsync(new MailMessageDbo
        {
            AccountId = account.AccountId,
            ThreadId = threadId,
        ...
        await WaitUntilAsync(() => viewModel.SelectedMessage != null);
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(viewModel.Messages, Has.Count.EqualTo(1));
            Assert.That(viewModel.Messages[0].MessageId, Is.EqualTo(messageId));
            Assert.That(viewModel.Messages[0].PlainTextBody, Is.EqualTo(HydratingMailProvider.HydratedBody));
            Assert.That(ReferenceEquals(viewModel.SelectedMessage, viewModel.Messages[0]), Is.True);
